...
foreach ($entries as $i => $e) {
    ...
    // PDO emulated prepares OFF (rule 12). Inline $isQueue as a literal so we
    // don't have to rebind 9+ :is_queue copies.
    $iq = $isQueue ? '1' : '0';
    $pdo->prepare("UPDATE om_companies SET
        ...
        logo_source_url      = :su,
        logo_png_path        = IF($iq = 1, logo_png_path,  IF(:is_png  = 1, :rel_png,  NULL)),
        logo_svg_path        = IF($iq = 1, logo_svg_path,  IF(:is_svg  = 1, :rel_svg,  NULL)),
        logo_webp_path       = IF($iq = 1, logo_webp_path, IF(:is_webp = 1, :rel_webp, NULL)),

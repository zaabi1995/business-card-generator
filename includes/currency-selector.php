...
<div class="relative" x-data="{ open: false }" @click.away="open = false">
    <button type="button"
            @click="open = !open"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg"
    ...
    <div x-show="open"
         x-transition
         class="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 z-50 py-1"
         style="display:none;">
        <?php foreach ($all as $code => $name): ?>
        <button type="button"
                data-currency="<?= htmlspecialchars($code) ?>"
                class="currency-option w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center justify-between <?= $code === $currentCurrency ? 'bg-blue-50' : '' ?>"

...
- [ ] 521. admin/employees.php employee add/edit modal + import-CSV modal + delete-confirm + regenerate-confirm + all other below-the-fold blocks (~700 strings). Cover every form field label, placeholder, helper text, validation message; CSV import wizard headers/hints; card-history sidebar; per-employee action dropdown items. Shipped as its own dedicated commit once the above-the-fold pass is in production.

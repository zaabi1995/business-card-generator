...
?>
<section class="my-8 rounded-2xl overflow-hidden border" style="background: <?= esc($bg) ?>12">
  <div class="flex flex-col md:flex-row items-center gap-6 p-6 md:p-8">
    <div class="w-48 h-48 flex items-center justify-center bg-white rounded-xl shadow-sm flex-shrink-0">
      <?php if ($src): ?>
        <img src="<?= esc($src) ?>" alt="<?= esc($company['name_en']) ?> logo"
             class="max-h-full max-w-full object-contain">
      <?php else: ?>
        <div class="text-gray-400 text-4xl font-bold">
          <?= esc(mb_substr($company['name_en'] ?? '??', 0, 2)) ?>
        </div>
      <?php endif; ?>
    </div>
    <div class="flex-1 text-center md:text-left">
      <div class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium"
           style="color: <?= esc($badge['color']) ?>; background: <?= esc($badge['color']) ?>15">
        <span class="w-1.5 h-1.5 rounded-full" style="background: <?= esc($badge['color']) ?>"></span>
        <?= esc($badge['label']) ?>
      </div>
      ...
      <?php if ($src): ?>
        <div class="mt-4 flex flex-wrap gap-2 justify-center md:justify-start">
          <?php if ($canDownload): ?>
            ...
            ?>
              <?php if ($available): ?>
                <a href="/logo-download?company=<?= (int) ($company['id'] ?? 0) ?>&format=<?= esc($fmt) ?>"

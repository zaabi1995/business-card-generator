...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        $message = t('orderdetail.invalid_request');
        $messageType = 'error';
    }
    $action = $_POST['action'] ?? '';
    ...
    if ($messageType === 'error') {
        ...
        if (isset($_FILES['po_file']) && $_FILES['po_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $ORDER_DOC_EXTS, true)) {
                ...
                $message = t('orderdetail.po_uploaded_ok');
                
                // Refresh order data
                $stmt->execute([$orderId]);
                $order = $stmt->fetch(PDO::FETCH_ASSOC);
            } else {
                $message = t('orderdetail.po_upload_failed');
                $messageType = 'error';
            }
        }
        ...
        $pdo->prepare("UPDATE print_orders SET quotation_accepted = 1, quotation_accepted_at = NOW() WHERE id = ?")
            ->execute([$orderId]);
        $message = t('orderdetail.quotation_accepted');
        ...
        $pdo->prepare("UPDATE print_orders SET quotation_requested = 1 WHERE id = ?")
            ->execute([$orderId]);
        $message = t('orderdetail.quotation_requested');
...
<div class="mb-6">
    <a href="print.php?tab=orders" class="text-blue-600 hover:text-blue-700 flex items-center gap-2">
        ...
        <?= htmlspecialchars(t('orderdetail.back_to_orders')) ?>
    </a>
</div>
...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="grid md:grid-cols-3 gap-4 pt-4 border-t border-gray-100">
                <div>
                    <p class="text-sm text-gray-500"><?= htmlspecialchars(t('orderdetail.print_shop')) ?></p>
                    <p class="font-semibold text-gray-900"><?php echo sanitize($order['print_shop_name'] ?? t('orderdetail.na')); ?></p>
                </div>
                <div>
                    <p class="text-sm text-gray-500"><?= htmlspecialchars(t('orderdetail.quantity')) ?></p>
                    <p class="font-semibold text-gray-900"><?= htmlspecialchars(str_replace(':n', (string) $order['quantity'], t('orderdetail.n_cards'))) ?></p>
                </div>
                <div>
                    <p class="text-sm text-gray-500"><?= htmlspecialchars(t('orderdetail.total')) ?></p>
        ...
        <!-- Card Details -->
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.card_details_h3')) ?></h3>
            <div class="grid md:grid-cols-2 gap-6">
                <?php if (!empty($order['card_front_url'])): ?>
                <div>
                    <p class="text-sm font-medium text-gray-700 mb-2"><?= htmlspecialchars(t('orderdetail.card_front')) ?></p>
                ...
                <div>
                    <p class="text-sm font-medium text-gray-700 mb-2"><?= htmlspecialchars(t('orderdetail.card_back')) ?></p>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
                <p class="text-sm text-gray-500"><?= htmlspecialchars(t('orderdetail.employee')) ?></p>
        ...
        <!-- Documents Section -->
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.documents_h3')) ?></h3>
                ...
                <div class="p-4 rounded-lg border border-gray-200 <?php echo !empty($order['quotation_file_path']) ? 'bg-green-50 border-green-200' : 'bg-gray-50'; ?>">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900"><?= htmlspecialchars(t('orderdetail.quotation')) ?></p>
                        ...
                        <div class="flex items-center gap-2">
                            <?php if (!empty($order['quotation_file_path'])): ?>
                                <a href="<?php echo getBasePath() . $order['quotation_file_path']; ?>" download
                                   class="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium flex items-center gap-1">
                                    <i class="fa-solid fa-download"></i> <?= htmlspecialchars(t('orderdetail.download')) ?>
                                </a>
                                ...
                                <form method="post" class="inline">
                                    ...
                                    <button type="submit" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">
                                        <?= htmlspecialchars(t('orderdetail.accept')) ?>
                                    </button>
                                </form>
                                <?php else: ?>
                                <span class="px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-sm font-medium">
                                    <i class="fa-solid fa-check"></i> <?= htmlspecialchars(t('orderdetail.accepted')) ?>
                                </span>
                                <?php endif; ?>
                            <?php elseif (empty($order['quotation_requested'])): ?>
                                <form method="post">
                                    ...
                                    <button type="submit" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">
                                        <?= htmlspecialchars(t('orderdetail.request_quotation')) ?>
                                    </button>
                                </form>
                            <?php else: ?>
                                <span class="px-3 py-1.5 bg-amber-100 text-amber-700 rounded-lg text-sm font-medium">
                                    <i class="fa-solid fa-clock"></i> <?= htmlspecialchars(t('orderdetail.requested')) ?>
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="p-4 rounded-lg border border-gray-200 <?php echo !empty($order['po_file_path']) ? 'bg-green-50 border-green-200' : ($order['po_required'] ? 'bg-amber-50 border-amber-200' : 'bg-gray-50'); ?>">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900"><?= htmlspecialchars(t('orderdetail.purchase_order')) ?></p>
                                ...
                                <p class="text-sm text-amber-600"><?= htmlspecialchars(t('orderdetail.po_required')) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <?php if (!empty($order['po_file_path'])): ?>
                                ...
                                <span class="px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-sm font-medium">
                                    <i class="fa-solid fa-check"></i> <?= htmlspecialchars(t('orderdetail.po_approved')) ?>
                                </span>
                                <?php endif; ?>
                            <?php else: ?>
                ...
                <div class="p-4 rounded-lg border border-gray-200 <?php echo !empty($order['invoice_file_path']) ? ($order['invoice_paid'] ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200') : 'bg-gray-50'; ?>">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900"><?= htmlspecialchars(t('orderdetail.invoice')) ?></p>
                                <?php if (!empty($order['invoice_number'])): ?>
                                <p class="text-sm text-gray-500">#<?php echo sanitize($order['invoice_number']); ?></p>
                                <?php else: ?>
                                <p class="text-sm text-gray-400"><?= htmlspecialchars(t('orderdetail.pending')) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-sm font-medium">
                                <i class="fa-solid fa-check"></i> <?= htmlspecialchars(t('orderdetail.paid')) ?>
                            </span>
                            <?php else: ?>
                            <span class="px-3 py-1.5 bg-amber-100 text-amber-700 rounded-lg text-sm font-medium">
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-200 flex items-center gap-4 text-sm">
                        <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.amount_prefix')) ?> <strong class="text-gray-900"><?php echo Currency::format($order['invoice_amount'], $currency); ?></strong></span>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="p-4 rounded-lg border border-green-200 bg-green-50">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900"><?= htmlspecialchars(t('orderdetail.delivery_note')) ?></p>
    ...
    <div class="space-y-6">
        <!-- Order Summary -->
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.order_summary_h3')) ?></h3>
            <div class="space-y-3 text-sm">
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.paper_type')) ?></span>
                    <span class="font-medium"><?php echo ucfirst($order['paper_type'] ?? 'standard'); ?></span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.finish')) ?></span>
                    <span class="font-medium"><?php echo ucfirst(str_replace('_', ' ', $order['finish'] ?? 'standard')); ?></span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.quantity')) ?></span>
                    <span class="font-medium"><?= htmlspecialchars(str_replace(':n', (string) $order['quantity'], t('orderdetail.n_cards'))) ?></span>
                </div>
                <?php if ($order['express_delivery']): ?>
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.delivery')) ?></span>
                    <span class="font-medium text-amber-600"><?= htmlspecialchars(t('orderdetail.express')) ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.subtotal')) ?></span>
                    <span class="font-medium"><?php echo Currency::format($order['subtotal'] ?? 0, $currency); ?></span>
                </div>
                <?php if (($order['setup_fee'] ?? 0) > 0): ?>
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.setup_fee')) ?></span>
                    <span class="font-medium"><?php echo Currency::format($order['setup_fee'], $currency); ?></span>
                </div>
                <?php endif; ?>
                <div class="flex justify-between">
                    <span class="text-gray-500"><?= htmlspecialchars(t('orderdetail.shipping')) ?></span>
                ...
                <div class="flex justify-between text-lg">
                    <span class="font-semibold text-gray-900"><?= htmlspecialchars(t('orderdetail.total')) ?></span>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.shipping_address_h3')) ?></h3>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.tracking_h3')) ?></h3>
            <div class="p-4 bg-cyan-50 border border-cyan-200 rounded-lg">
                <p class="text-sm text-gray-500 mb-1"><?= htmlspecialchars(t('orderdetail.tracking_number')) ?></p>
        ...
        <!-- Print Shop Contact -->
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><?= htmlspecialchars(t('orderdetail.print_shop_h3')) ?></h3>
            <p class="font-medium text-gray-900"><?php echo sanitize($order['print_shop_name'] ?? t('orderdetail.na')); ?></p>
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full">
            ...
            <form method="post" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2"><?= htmlspecialchars(t('orderdetail.field_po_number')) ?></label>
                    <input type="text" name="po_number" required
                           ...
                           placeholder="<?= htmlspecialchars(t('orderdetail.po_number_ph')) ?>">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2"><?= htmlspecialchars(t('orderdetail.field_po_document')) ?></label>
                    ...
                    <p class="text-xs text-gray-500 mt-1"><?= htmlspecialchars(t('orderdetail.po_document_hint')) ?></p>
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <button type="submit"
                        ...
                        <?= htmlspecialchars(t('orderdetail.upload_po')) ?>
                    </button>
                </div>
            </form>
        </div>

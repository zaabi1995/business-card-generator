    ...
    <div>
      ...
      <div className="overflow-hidden rounded-card bg-surface shadow-card">
        <div className="primitive-table-cell flex items-center gap-4 border-b border-line-strong">
          <div className="min-w-0 flex-[2]"><Th k="name" label="Company" /></div>
          <div className="hidden min-w-0 flex-[2] md:block">
            <span className="text-[12px] font-medium text-ink-2">Admin</span>
          </div>
          <div className="w-24 shrink-0"><Th k="employee_count" label="People" /></div>
          <div className="w-24 shrink-0"><Th k="order_count" label="Orders" /></div>
          <div className="w-32 shrink-0"><Th k="total_spend" label="Spend" /></div>
        </div>

        {shown.length === 0 ? (
          <p className="px-6 py-14 text-center text-[13px] text-ink-2">
            {rows.length === 0 ? "No companies yet" : "Nothing matches that filter"}
          </p>
        ) : (
          shown.map((r) => {
        ...
        {shown.length > 0 && (
          <div className="primitive-card-footer flex items-center gap-4 border-t border-line-strong bg-inset">
            <span className="flex-[2] text-[12px] text-ink-3">
              ...
              {shown.length === rows.length ? " companies" : " of " + rows.length}
            </span>
            ...
            <span className="w-32 shrink-0 text-[11px] text-ink-3">mixed currencies</span>
          </div>
        )}
      </div>
    </div>
